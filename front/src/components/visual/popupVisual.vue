<template>
    <div class="d">
        <div class="header">           
            <svg t="1694154128190" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="7915" width="200" height="200"><path d="M892.181695 850.874576a176.162712 176.162712 0 0 0-63.435932-12.235932c-47.728814 0-73.762712 16.141017-108.474577 54.410848l-3.471186 3.991864v-178.766102h153.6V616.135593H716.8V495.945763h153.6V393.458983H716.8V273.529492h153.6v-102.4H716.8V0H614.4v887.235254c-31.761356-34.191186-57.708475-48.59661-103.181017-48.59661-51.460339 0-78.101695 18.484068-116.024407 63.002034-21.434576 25.079322-28.029831 29.678644-42.782373 29.678644s-21.347797-4.599322-42.782372-29.678644c-38.096271-44.517966-64.477288-63.002034-115.937628-63.002034a207.056271 207.056271 0 0 0-120.10305 38.53017 197.944407 197.944407 0 0 0-40.612882 36.447457Q16.141017 929.670508 0 945.898305l70.117966 62.828475c6.595254-7.029153 17.96339-18.397288 31.327458-30.89356l5.03322-5.640678a122.012203 122.012203 0 0 1 87.300339-41.220339c14.665763 0 21.347797 4.599322 42.782373 29.678644 38.183051 44.517966 64.477288 63.002034 115.937627 63.002034s78.101695-18.570847 116.024407-63.002034c21.434576-25.079322 28.029831-29.678644 42.782373-29.678644s21.347797 4.599322 42.695593 29.678644S616.135593 1024 676.881356 1024c52.067797 0 84.43661-34.711864 108.995254-63.002034 21.434576-25.079322 28.11661-29.678644 42.869153-29.678644 21.521356 0 49.898305 13.190508 79.40339 36.100339a338.440678 338.440678 0 0 1 42.695593 39.918644l73.155254-59.357288q-82.093559-75.237966-131.818305-97.106441z" p-id="7916"></path><path d="M200.027119 445.179661C182.237288 482.321356 173.559322 512.867797 173.559322 535.864407c0 104.135593 84.870508 188.485424 190.915254 192.564068 106.044746-4.078644 190.915254-88.602034 190.915255-192.564068 0-22.99661-8.677966-53.543051-26.033899-90.684746a838.031186 838.031186 0 0 0-58.663051-101.879322c-22.562712-34.104407-45.125424-64.824407-60.138305-84.52339-17.355932-22.649492-29.071186-37.054915-30.63322-38.964068a18.136949 18.136949 0 0 0-30.025763 0c-1.562034 1.909153-13.450847 16.314576-30.63322 38.964068-15.012881 19.698983-37.575593 50.418983-60.138305 84.52339a838.031186 838.031186 0 0 0-59.096949 101.879322z m176.249491 167.745085a97.36678 97.36678 0 0 0 60.745763-89.209492A22.475932 22.475932 0 0 1 459.932203 501.412881a22.389153 22.389153 0 0 1 22.389153 22.475933 142.318644 142.318644 0 0 1-88.949153 130.169491 22.99661 22.99661 0 0 1-8.677966 1.735593 22.475932 22.475932 0 0 1-8.677966-43.38983zM156.810847 286.372881A106.131525 106.131525 0 0 0 260.338983 182.237288a124.615593 124.615593 0 0 0-14.318644-49.204068 467.915932 467.915932 0 0 0-31.848136-55.191864 834.117424 834.117424 0 0 0-32.629152-45.906441C172.257627 19.351864 165.749153 11.454915 164.881356 10.500339a9.719322 9.719322 0 0 0-16.227797 0c-0.867797 0.954576-7.376271 8.677966-16.661695 21.087458-8.157288 10.673898-20.39322 27.335593-32.629152 45.90644a467.915932 467.915932 0 0 0-31.848136 55.191865A124.615593 124.615593 0 0 0 53.195932 182.237288a106.131525 106.131525 0 0 0 103.614915 104.135593z m6.334916-62.654915a52.848814 52.848814 0 0 0 33.23661-48.423051A12.149153 12.149153 0 0 1 208.271186 163.232542a12.149153 12.149153 0 0 1 12.149153 12.149153 77.233898 77.233898 0 0 1-48.249492 70.812203 12.930169 12.930169 0 0 1-4.686101 0.954577 12.149153 12.149153 0 0 1-4.686102-23.430509z" p-id="7917"></path></svg>
            <text>潮位数据详情</text>
        </div>

        <div class="back-shadow">
            <div class="Position">
                <div class="card">
                    <div class="flex1">
                        <text class="larger">
                            当前位置:
                        </text>
                        <div class="one-line-info">
                            <text class="default">经度:</text>
                            <div class="data">{{info.lng}}</div>
                        </div>
                        <div class="one-line-info">
                            <text class="default">纬度:</text>
                            <div class="data">{{info.lat}}</div>
                        </div>

                        <div style="display: flex;margin-top: 10px;">
                            <text class="larger">预报水位：</text>
                            <div class="data">{{info.water}}</div>
                        </div>
                    </div>

                    <div class="flex2">
                        <svg t="1694156221548" class="_svg" viewBox="0 0 1025 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="11639" width="200" height="200"><path d="M1025.1 568c0-24.2-13.7-46.4-35.4-57.2l-202.3-101c-10.8-5.4-17.7-16.5-17.7-28.6V172.7c0-26.7-16.6-50.6-41.6-59.9L550.5 46.3c-24.2-9-50.8-9-74.9 0.1l-176.9 66.8c-24.9 9.4-41.4 33.3-41.4 59.9v207.1c0 12.1-6.9 23.2-17.7 28.6L35.5 510.7C13.8 521.5 0 543.8 0.1 568.1l0.5 226c0 23.2 12.6 44.5 32.9 55.8l223.8 124.6 1.2 0.7c18.9 10.5 41.9 10.4 60.8-0.2l2-1.1L497 873.2c9.9-5.7 22.1-5.7 32 0.1l176.8 102.5c9.5 5.4 20.1 8.1 30.7 8.1 10.4 0 20.9-2.6 30.3-7.8l3-1.7L992.4 851c20.4-11.3 33-32.8 33-56.1l-0.3-226.9zM705.6 379.8c0 12.2-6.8 23.3-17.7 28.7l-131.4 65.7c-5.3 2.7-11.6-1.2-11.6-7.2V304.1c0-29.1 17.7-55.2 44.7-66L705 192l0.6 187.8zM257.5 621l-0.2-0.1-149.1-75 149.1-74.4 16.8-8.4c9.2-4.6 20-4.5 29.1 0.3l18.1 9.4 129.8 64.3c5.8 2.9 6 11.1 0.2 14.2l-130 69.8c-20 10.1-43.7 10-63.8-0.1z m205.7 203.9l-141.8 70.6-0.8-163.6c-0.1-27 15.1-51.8 39.2-63.9l108.8-54.5c5.3-2.7 11.6 1.2 11.6 7.1l0.8 175.6c0 12.1-6.9 23.2-17.8 28.7zM488 182.6L385 144l128-48 128 48-103 38.6c-16.1 6.1-33.9 6.1-50 0z m215.7 439.2l-129.2-70.5c-5.7-3.1-5.5-11.3 0.3-14.2l131-65.3 16.1-8.4c9.1-4.8 20-4.9 29.1-0.3l18.7 9.3L916.9 546l-146.2 76.5c-21.1 10.9-46.2 10.7-67-0.7zM961 780.7c0 11.8-6.5 22.7-17 28.3l-174.3 92.2-0.6-169.1c-0.1-27 15.1-51.8 39.3-63.8L961 592v188.7z" fill="#1296db" p-id="11640"></path></svg>
                    </div>           
                </div>
                
            
            </div>


            <div class="Water">
            <div class="card">
                <text class="larger">
                    临近站点：        
                </text>

                <div class="flex">
                    <div class="flex_1">
                    <div class="one-Satation-info">
                        <div style="display: flex;"><text class="default">站点:</text><div class="data">{{props.nearStations![0]!.name}}</div></div>
                        <div style="display: flex;"><text class="default">实测水位:</text><div class="data">{{props.nearStations![0]!.water}}</div></div>
                    </div></div>
                
                    
                    <div class="flex_2">
                      <div class="one-Satation-info">
                        <div style="display: flex;"><text class="default">站点:</text><div class="data">{{props.nearStations![1]!.name}}</div></div>
                        <div style="display: flex;"><text class="default">实测水位:</text><div class="data">{{props.nearStations![1]!.water}}</div></div>
                      </div>
                    </div>

                </div>

            </div>
            </div>
            
        </div>
    </div>
</template>
<script setup lang="ts">
import { WaterStation } from '@/type';
import {computed} from 'vue';

const props = defineProps({
    lnglat:Object,
    water:Number,
    nearStations:Array
})

const info = computed(()=>{
    return {
        lng : props.lnglat!.lng.toFixed(4),
        lat : props.lnglat!.lat.toFixed(4),
        water : props.water?.toFixed(4),
    }
})


</script>
<style lang="scss" scoped>
.d {
    width: 400px;
    height: auto;

    .back-shadow{
        width: 95%;
        border:#dbdbdb50 1px solid;
        // border-radius: 25px;
        background:#ecf5ffaf;
    

        box-shadow:0px 0px 5px rgba(65, 105, 225, 0.315);
    }
    .header{
        line-height: 30px;
        display: flex;
        width: 90%;
        text{
            font-size: 22px;
            font-weight: 700;
            color: #303133;
            top:0%;
            align-items: center;
            padding: 5px;
            
        }
        svg{
            width: 2rem;
            height: 2rem;
            fill:royalblue;
            padding: 5px;
        }
    }

    .Position{
        padding-left: 10px;
        .card{
            width: 95%;
            display: flex;
            .flex1{
                .one-line-info{
                    display: flex;
                    width: 200px;
                }
                }
            
            .flex2{
                margin-left: 20px;
                ._svg{
                    width: 5rem;
                    height: 5rem;
                }
            }

        }
    }

    .Water{
        padding-left: 10px;
        .card{
            width: 95%;
            .flex{
                display: flex;
                .flex_1{
                    flex: 1;
                }
                .flex_2{
                    flex: 1;
                }
            }
            
            .one-Satation-info{
                width: 200px;

            }
        }
        
    }   
}

text.larger{
    font-size: 18px;
    font-weight: 600;
}
text.default{
    font-size: 16px;
}
.data{
    width: 30%;
    font-size: 16px;
}
.card{
    margin-top: 1rem;
    margin-bottom: 1rem;
}
</style>